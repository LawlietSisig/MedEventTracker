...
if ($search !== '') {
    $where[]  = '(p.first_name LIKE ? OR p.last_name LIKE ? OR p.contact_number LIKE ?)';
    $like     = "%{$search}%";
    $params[] = $like;
    $params[] = $like;
    $params[] = $like;
...
function getAge(string $dob): string {
    if (!$dob) return 'N/A';
    try {
        $bday = new DateTime($dob);
        $today = new DateTime('today');
        return (string)$bday->diff($today)->y . ' yrs';
    } catch (Exception $e) {
        return 'N/A';
    }
}
...
<head>
    ...
    <meta name="description" content="Patients — Medical Outreach Tracker">
    <title>Patients — Medical Outreach Tracker</title>
...
<div class="dashboard-layout">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="patients.php" class="nav-link active" id="nav-patients">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <div class="page-content">
            ...
            <div class="oe-controls" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: var(--space-6); flex-wrap:wrap; gap: var(--space-4);">
                <form method="GET" action="" class="oe-search-form" id="search-form" style="display:flex; flex:1; max-width: 400px; position:relative;">
                    ...
                    <input type="text" class="form-input" name="search" id="search-input" placeholder="Search patients by name or phone..." value="<?php echo htmlspecialchars($search); ?>" style="padding-left:2.8rem; border-radius:var(--radius-full);">
                </form>
            ...
            <div class="oe-empty" style="text-align:center; padding:var(--space-12) 0; color:var(--slate-500);">
                ...
                <p>Register your first patient to start managing records.</p>
                <?php if (canAdmin($user)): ?>
                <button class="btn-primary" onclick="openCreateModal()" style="margin-top:var(--space-4);width:auto;padding:var(--space-3) var(--space-6);">Register Patient</button>
                <?php endif; ?>
            </div>
            <?php else: ?>
                ...
                <div class="oe-card" style="background:white; border-radius:var(--radius-xl); border:1.5px solid var(--slate-200); overflow:hidden; box-shadow:var(--shadow-sm); display:flex; flex-direction:column; transition:all var(--transition-base);">
                    <div style="padding:var(--space-5); border-bottom:1px solid var(--slate-100);">
                        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:var(--space-2);">
                            <h3 style="font-family:var(--font-display); font-size:1.15rem; font-weight:700; color:var(--slate-900);"><?php echo htmlspecialchars($p['first_name'] . ' ' . $p['last_name']); ?></h3>
                            <span style="font-size:0.8rem; font-weight:600; color:var(--primary-600); background:rgba(14, 165, 165, 0.1); padding:2px 8px; border-radius:var(--radius-full);">
                        ...
                        <div style="font-size:0.9rem; color:var(--slate-500); display:flex; flex-direction:column; gap:4px;">
                            <div style="display:flex; align-items:center; gap:6px;">
                                ...
                                <span><?php echo date('M j, Y', strtotime($p['dob'])); ?> (<?php echo getAge($p['dob']); ?>)</span>
                            </div>
                            ...
                            <div style="display:flex; align-items:center; gap:6px;">
                                ...
                                <span><?php echo htmlspecialchars($p['contact_number']); ?></span>
                            </div>
                            <?php endif; ?>
                            <?php if ($p['blood_type']): ?>
                            <div style="display:flex; align-items:center; gap:6px; margin-top:4px;">
                                <span style="font-weight:700; color:var(--slate-700); background:var(--slate-100); padding:2px 6px; border-radius:4px; font-size:0.75rem;">Blood: <?php echo htmlspecialchars($p['blood_type']); ?></span>
                            </div>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php if ($p['medical_notes']): ?>
                    <div style="padding:var(--space-4) var(--space-5); background:var(--slate-50); flex:1; font-size:0.85rem; color:var(--slate-600); border-bottom:1px solid var(--slate-100);">
                        ...
                        <?php echo nl2br(htmlspecialchars($p['medical_notes'])); ?>
                    </div>
                    <?php else: ?>
                    ...
                    <div style="padding:var(--space-3) var(--space-5); display:flex; justify-content:space-between; align-items:center; background:white;">
                        <span style="font-size:0.75rem; color:var(--slate-400);">Added <?php echo date('M j', strtotime($p['created_at'])); ?></span>
                        <?php if (canAdmin($user)): ?>
                        <div style="display:flex; gap:8px;">
                            <button class="oe-action-btn oe-action-edit" title="Edit patient"
                            ...
                            <button class="oe-action-btn oe-action-danger" title="Delete patient"
                                onclick="confirmDelete(<?php echo $p['id']; ?>, '<?php echo htmlspecialchars(addslashes($p['first_name'] . ' ' . $p['last_name'])); ?>')"
    ...
    <!-- ═══ CREATE / EDIT MODAL ═══ -->
    <div class="modal-overlay" id="patient-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
        <div class="modal-box">
            <div class="modal-header">
                <h3 id="modal-title">New Patient Profile</h3>
                <button class="modal-close" onclick="closeModal()" aria-label="Close">&times;</button>
            </div>
            <form id="patient-form" method="POST" action="../handlers/patient_handler.php" novalidate>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-input form-select modal-input" id="f-gender" name="gender" required>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label class="form-label" for="f-blood">Blood Type</label>
                            <select class="form-input form-select modal-input" id="f-blood" name="blood_type">
    ...
    <div class="modal-overlay" id="delete-modal" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
        <div class="modal-box modal-box-sm">
            ...
            <div class="modal-body">
                <p style="color:var(--slate-600); line-height:1.5; margin-bottom:var(--space-2);">Are you sure you want to delete the record for <strong id="delete-patient-name" style="color:var(--slate-900);"></strong>?</p>
                <p style="color:var(--slate-500); font-size:0.85rem;">This action cannot be undone and will permanently remove this patient's medical history.</p>
            </div>
            <div class="modal-footer" style="border-top:none; background:transparent;">
                ...
                <form method="POST" action="../handlers/patient_handler.php" style="display:inline;">
                    <input type="hidden" name="action" value="delete">
                    <input type="hidden" name="patient_id" id="delete-patient-id">
                    <button type="submit" class="btn-danger" id="confirm-delete-btn">Delete Record</button>
                </form>
            </div>
        </div>
    </div>
...
<script src="../assets/js/patients.js?v=<?php echo time(); ?>"></script>
</body>
</html>
